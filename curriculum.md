...
- Live demo of Plato answering a multi-part rule interpretation question: annual hull survey scope, suspect areas, and close-up survey expectations for a bulk carrier
- Plato returns structured answer with citations and clickable links to ABS Survey After Construction Rules (Part 7, Chapter 3, Section 2), distinguishing between non-double-skin and double-skin bulk carrier requirements
- Follow-up scenario: suspect area with substantial corrosion and coating breakdown in a ballast tank — Plato connects two rules and returns a survey prep game plan (re-examination, thickness measurements, potential gauging extension)
- References go to subsection level (1.1.9 suspect areas, 1.1.10 ballast tank requirements)
...
- Live build of a workspace for engineering environmental plan reviews — context generated by Sierra, pasted into workspace; each plan type gets its own task mode
- Workspace context: general environmental plan review guidance, file-only sourcing (no internet), output style — generated in Markdown for easy LLM consumption
- Task mode built for SOPEP review: 34 existing review questions fed into Sierra, which generates the full task mode instructions; pasted into workspace
- Demo run: SOPEP file uploaded, task mode invoked, output includes summary and a compliance checklist (yes/no per question, finding, source reference) plus a reviewer check for human sign-off
- Key point: one workspace, multiple task modes — general context and style consistent across all plan types, specific instructions per plan

**Day 21: Web Search and Deep Research**
- Live demo of web search and deep research tools applied to a current maritime topic: alternative fuel pathways (ammonia, methanol, LNG)
- Step 1 — web search: structured comparison across five dimensions, executive summary, and comparison table with source citations
- Step 2 — judgment layer: trade-offs, risks, and uncertainties per fuel in five bullets; Sierra frames the issue for decision support, separating near-term commercial readiness from long-term decarbonization potential
- Step 3 — deep research: full technical report with detailed citations, downloadable, reusable for briefings and client-facing materials
- Bottom line from demo: LNG most operationally established, methanol has near-term momentum, ammonia has long-term potential but significant hurdles
...
- Live demo of Sierra's built-in HR policy repository covering ABS Bureau, ABS Group, and Wavesight across dozens of countries
- Sierra automatically applies the policy for the user's entity and home country — no need to specify; every employee gets answers tailored to their own profile
- Demos: PTO query (US Group employee), parental leave for Bureau employee in Norway, travel policy for ABS Group US
- Sierra retrieves only from HR documents — no external sourcing — and cites exactly where each answer came from
- For complex or edge-case questions: if the policy isn't in the repository, Sierra says so clearly and directs the user to their HR business partner rather than guessing
- Series close: certification through iAchieve; reach out to the AI Center of Excellence for support
