# SIERRA Literacy Month — Week 5 Slides
## Days 19–22: Putting It Together
...
- When output isn't right: refine the prompt, don't restart — add specificity or give Sierra an example of what you want
- Always review outputs before sending — Sierra is your first draft engine, not your final editor

[Visual: End-to-end workflow diagram for a real ABS scenario. Inputs on the left: SURVEY REPORT (file icon) + CLIENT CONTEXT (workspace icon) + "SUMMARIZE FOR CLIENT" Task Mode. These feed into SIERRA → CLIENT SUMMARY (output). Below the diagram: a habit callout — "Focused thread. Specific prompt. Verify before sending."]
...
[Visual: Research workflow diagram. Three retrieval inputs on the left — PLATO, WEB SEARCH, PROJECT FILES — all feed into CONTEXT → MODEL → SYNTHESIZED ANSWER WITH SOURCES. A separate callout box: "Always verify: specific facts, statistics, citations, and anything near the knowledge cutoff." Makes the validate step feel like part of the workflow, not an afterthought.]
...
[Visual: A clean journey map — horizontal timeline with five labeled milestones, one per week: Week 1 "Why SIERRA" → Week 2 "How LLMs Work" → Week 3 "Prompting & How Sierra Works" → Week 4 "Knowledge Systems & Tools" → Week 5 "Full Workflows." Simple icons or colors for each week. Ends with a final label: "Now you're ready." Satisfying visual close to the series.]
